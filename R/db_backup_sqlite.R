...
#' @param datetime_col which column is datetime
...
#' exclude_columns = c("satellite",  "messageDate", "locationClass", "compressionIndex",  "S1", "S2", "S3", "S4", "S5", "S6", "S7", "S8", "filenam", "pk") , 
#' indices = c('tagID', 'locationDate')
#' datetime_cols = 'locationDate'
#' )
#' }
#' 
sqlitedump <- function(db, tables,exclude_columns, indices,datetime_cols,
                      dir = '.',
    ...
    xl = foreach(i=1:length(tables)) %do% {
      ...
      if (!missing(datetime_col)) {
        ti[, (datetime_col) := as.character(datetime_col)]
      }
    ...
    conlite = dbConnect(RSQLite::SQLite(), dbname = paste(dir, filename,sep='/'), extended_types = TRUE)
